...
import { UserCheck, Building2, Plus, Settings as SettingsIcon } from 'lucide-react';
...
const App = () => {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      if (config) {
        ...
        setTimeout(() => {
          // Load sessions after sites are loaded
  ...
  useEffect(() => {
    if (isLoaded && sites.length >= 0) {
      saveToStorage('capacity-sites-config', {
        sites,
        globalSettings,
  ...
  // Polling loop - only start after sites are loaded
  useEffect(() => {
  ...
  }, [sites, globalSettings, isLoaded, pollSiteSwitches]);
  ...
  const pollSiteSwitches = useCallback(async (site) => {
    const now = new Date();
    const allDevices = [];

    for (const switchStack of site.switches) {
      const devices = await pollSwitch(
        switchStack, 
        site.excludedVlans || [], 
        site.excludedPorts || []
      );
    ...
    detectedMacs.forEach(mac => {
      ...
      if (newSessions.has(mac)) {
        // Update existing session - keep original firstSeen
        ...
        // New session - set firstSeen to now
        newSessions.set(mac, {
          mac,
          firstSeen: now,
          lastSeen: now,
  ...
  const addSite = () => {
    const newSite = {
      ...
      switches: [],
      enabled: true,
      excludedVlans: [],  // Add VLAN exclusion list
      excludedPorts: []   // Add port exclusion list
    };
